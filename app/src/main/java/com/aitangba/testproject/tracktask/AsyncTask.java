...
abstract class AsyncTask<Params, Progress, Error, Result> {
    ...
    private final AtomicBoolean mCancelledWithoutCallback = new AtomicBoolean();
    ...
    @MainThread
    protected void onError(Error... values) {
    }
    ...
    public final boolean cancelWithoutCallback() {
        mCancelled.set(true);
        mCancelledWithoutCallback.set(true);
     ...
     */
    @SafeVarargs
    public final AsyncTask<Params, Progress, Error, Result> executeParallel(Params... params) {
     ...
     */
    @SafeVarargs
    protected final AsyncTask<Params, Progress, Error, Result> executeSerial(Params... params) {
     ...
     */
    @MainThread
    public final AsyncTask<Params, Progress, Error, Result> executeOnExecutor(Executor exec,
                                                                       Params... params) {
    ...
    protected final void publishError(Error... values) {
        if (!isCancelled()) {
            getHandler().obtainMessage(MESSAGE_POST_ERROR,
                    new AsyncTaskResult<Error>(this, values)).sendToTarget();
        }
    }
    ...
    private static class InternalHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (result.mTask.mCancelledWithoutCallback.get()) {
                return;
            }
            switch (msg.what) {
